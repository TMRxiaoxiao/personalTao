...
Page({
  ...
  onLoad: function (options) {
    console.log(options);
    var data1 = { token: getApp().globalData.token, id: options.id};
    if(typeof(options.shopid)!=='undefined'){
        data1.shopid = options.shopid;
    }
    ...
    wx.request({
      ...
      success:res=>
      {
        ...
        if(options.promotype=="特卖"){
          data.promo.type=1
          data.promo.disprice = options.dispr
        }
        this.setData({ detailed: data, totalNum: res.data.data.photoimgs.length })
        if (data.promo == null) {
          return false
        } else {
          this.setData({ activeBuy: data.promo.introduce })
        }
